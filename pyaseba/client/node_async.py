...
class NodeAsync(Node):
    ...
    async def connect(  # type: ignore[override]
        ...
        if self._client.is_connected or await self._client.connect(
            ...
            node = await self._client.wait_node_connection(node=node_id)
            if node is not None:
                self._node_id = node
                self._init()
                self._start()
    ...
    async def update(self  # type: ignore[override]
        ...
        assert (self._client)
        self._variable_values = await self._client.get_all_variables(self._node_id)
    ...
    async def wait(self, name: str) -> bool:
        assert (self._client)
